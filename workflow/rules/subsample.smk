...
#        
    ...
    input:
        ...
        interval_bed=config["pi_created_selected_bed"]
    envmodules:
        TOOLS["R"]["version"],
    ...
    output:
        selected_bed=join(RESULTSDIR,'03_aligned','02_bed','{sample_id}.{species}.{min_length}-{max_length}.{selection_shorthand}.bed')
    shell:
        """
...
rule selected_hist_frags:
    ...
    params:
        ...
        rscript=join(WORKDIR,"scripts","hist.r")
    output:
        hist=join(RESULTSDIR,'03_aligned','03_histograms','{sample_id}.{species}.length_hist.{min_length}-{max_length}.{selection_shorthand}.csv'),
        png=join(RESULTSDIR,'03_aligned','03_histograms','{sample_id}.{species}.length_hist.{min_length}-{max_length}.{selection_shorthand}.png')
    shell:
        """
